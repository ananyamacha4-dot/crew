...
class ProductBrief(BaseModel):
    ...
    app_type: AppType = Field(
        description=(
            "What kind of app to build. One of: " + ", ".join(ALL_APP_TYPES) + "."
        ),
    )
    primary_screen_goal: str = Field(
        default="",
        description=(
            "One sentence: what the main screen must let the user do. "
            "e.g. 'play snake with arrow keys', 'see sales metrics at a glance', "
            "'add items to a todo list'. This is the load-time experience — "
    ...
    specific_features: list[str] = Field(
        default_factory=list,
        description=(
            "Concrete things the user mentioned or that the app type requires. "
            "e.g. ['scientific functions', 'memory keys', 'history'] for a calculator."
        ),
    )
    ...
    # Legacy fields kept so older code paths (landing-page scaffold) still work.
    # They become OPTIONAL — the new pipeline doesn't require them.
    product_type: str = Field(default="", description="Deprecated; use app_type")
    pages: list[PageSpec] = Field(default_factory=list)
    interactions: list[InteractionSpec] = Field(default_factory=list)
    must_not: list[str] = Field(default_factory=list)

    @field_validator("mood", "must_not", "specific_features", mode="before")
    @classmethod
    ...
    def _coerce_app_type(cls, value: object) -> object:
        # Llama sometimes returns aliases or near-misses. Normalize.
        if not isinstance(value, str):
            return value
        v = value.strip().lower().replace("_", "-").replace(" ", "-")
        if v in ALL_APP_TYPES:
            return v
        # Exact aliases — try these before substring matching.
        aliases = {
            "saas": "dashboard",
            "saas-landing": "landing",
            "mobile-app-landing": "landing",
            "blog": "landing",
            "agency": "landing",
            "restaurant": "landing",
            "marketing": "landing",
        ...
        if v in aliases:
            return aliases[v]
        # Substring fallback — catches "snake-game", "tic-tac-toe-game", "sales-dashboard", etc.
        substring_map = [
            ("game", "game"),
            ("dashboard", "dashboard"),
            ("portfolio", "portfolio"),
        ...
        for needle, target in substring_map:
            if needle in v:
                return target
        # Unknown → safest fallback that still produces SOMETHING usable.
        return "landing"
...
class DesignSystem(BaseModel):
    ...
    shadow_style: Literal["flat", "subtle-paper", "soft-layered", "neon-glow"] = "soft-layered"
    animation_style: Literal["none", "smooth", "snappy", "playful", "graceful"] = "smooth"
